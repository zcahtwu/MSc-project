...
n_slices = 32
# for test set
test_all_score = np.zeros((int(length_test/n_slices),1))
...
for i, dataset in enumerate(testloader, 0):
    ...
    test_all_score[i,0] = torch.mean(outputs.cpu().detach().numpy()[0:n_slices])
...
for nth_try in range(1,101):
    ...
    # The saved FC net
    FC = f'./FCmodel/{nth_dcnn}/balance{nth_try}.pt'
    net = FCnet()
